...
class Guida extends React.Component {
  ...
  componentDidMount() {
    fetch(
      "http://localhost:4200/api/guides/like/get/" +
        this.props.autore +
        "&" +
        this.props.titolo,
    )
      ...
      .then((json) => this.checkLikes(json));
  }
    ...
    if (json.message.length > 0) {
      for (let el of json.message) {
        if (localStorage.getItem("normalEmail") != null)
          if (
            el.user === JSON.parse(localStorage.getItem("normalEmail")).username
          ) {
  ...
  render(props) {
    return (
      <div
        ...
        data-autore={this.props.autore}
        data-titolo={this.props.titolo}
        data-testo={this.props.testo}
        data-liked={this.state.liked}
      >
