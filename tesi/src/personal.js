...
class Personal extends React.Component {
  ...
  componentDidUpdate() {
    if (
      this.state.aux3.length !== this.state.guide.length &&
      this.state.guideOn === true
    )
  ...
  addGuide() {
    ...
    fetch(
      "http://localhost:4200/api/moderate/" +
        this.state.titolo +
        this.state.testo,
    )
      .then((response) => {
  ...
  updateGuide() {
    ...
    fetch("http://localhost:4200/api/guides/update", {
      ...
      this.setState(
        {
          message: "Vedrai aggiornata la tua guida al tuo prossimo ingresso",
          writing: false,
          guide: [],
        },
        () => this.fillAux3(),
  ...
  writeNewWithTitle(event) {
    this.setState({
      ...
      richieste: [],
      suggerimenti: [],
      nome: event.currentTarget.dataset.testo,
      writing: true,
      guideOn: false,
    });
  }
  ...
  writeUpdate(event) {
    this.setState({
      ...
      richieste: [],
      suggerimenti: [],
      nome: event.currentTarget.dataset.titolo,
      writing: true,
      actualText: event.currentTarget.dataset.testo,
      guideOn: false,
    });
  }
  ...
  render() {
    ...
    if (this.state.changingData === true) {
      data = (
        <div id="dataValues">
          <input
            placeholder={"Nuova  " + this.state.dataToChange}
            onChange={(event) =>
          ...
          <button
            onClick={() =>
              this.updateData(this.state.dataToChange, this.state.inputValue)
            }
          >
            Invia
          </button>{" "}
    ...
    if (this.state.guideOn === true)
      button = (
        <button id="Scrivi" onClick={() => this.writeNew()}>
          {" "}
    ...
    if (this.state.writing === true && this.state.changeGuide === false) {
      ...
      write = (
        <div className="writing">
          ...
          <textarea
            id="testo_guida"
            defaultValue={this.state.actualText}
            onChange={(event) => this.setState({ testo: event.target.value })}
          ></textarea>
        </div>
      );
      button = (
        <button onClick={() => this.updateGuide()}>Aggiorna guida </button>
      );
    }
    //Mostra dati
    ...
    return (
      <div id="container">
        ...
        {data}
        <nav>
          <div className="sec" onClick={this.showRequests.bind(this)}>
            {" "}
            Richieste{" "}
            <span className="number"> {this.state.aux1.length}</span>{" "}
          </div>
          <div className="sec" onClick={this.showSuggestions.bind(this)}>
            {" "}
            Suggerimenti{" "}
            <span className="number">{this.state.aux2.length}</span>{" "}
          </div>
          <div className="sec" onClick={this.showGuides.bind(this)}>
            {" "}
            Guide scritte{" "}
          </div>
          <div className="sec" onClick={this.showData.bind(this)}>
            {" "}
            Dati personali{" "}
          </div>
        ...
        <div className="richieste">
          {this.state.richieste.map((ric, index) => (
            <Request
              key={index}
              testo={ric.payload.richiesta}
              onClick={(event) => this.writeNewWithTitle(event)}
            />
          ))}
          ...
          {this.state.guide.map((ric, index) => (
            <Guida
              ...
              titolo={ric.titolo}
              autore={ric.autore}
              onClick={(event) => this.writeUpdate(event)}
            />
          ))}
