...
class Home extends React.Component {
  ...
  addComment() {
    ...
    fetch("http://localhost:4200/api/moderate/" + this.state.currentComment)
      .then((response) => {
      ...
      .then((json) => {
        if (json.ris === false) {
          this.setState({
            data: "Il tuo commento e' inappropriato e non verra' pubblicato!",
            comments: false,
            clicked: false,
          });
        } else {
  ...
  addGuide() {
    ...
    this.setState({
      data: "Abbiamo aggiunto questa guida al nostro database, grazie per il tuo contributo!",
      loading: "none",
    });
  }
  ...
  getApi() {
    ...
    if (!this.state.value.startsWith("Come fare a")) {
      this.setState({
        data: "La tua ricerca non ha prodotto risultati. Verifica di aver inserito 'Come fare a' come prime parole ",
      });
      this.setState({ loading: false });
      return;
    }

    fetch("http://localhost:4200/api/moderate/" + this.state.value)
      .then((response) => {
        return response.json();
      })
      .then((json) => {
        if (json.ris === false) {
          this.setState({
            data: "La tua richiesta viola determinati parametri. Non e' stato possibile rispondere",
          });
          this.setState({ loading: false, block: true }, () => {
            this.getApi2();
          });
        } else this.getApi2();
      });
  ...
  getApi2() {
    if (this.state.block === false)
      fetch("http://localhost:4200/api/find/" + this.state.value)
        .then((response) => {
          return response.json();
        })
        .then((json) => {
  ...
  callGPT() {
    fetch("http://localhost:4200/api/" + this.state.value)
      .then((response) => {
        return response.json();
      })
      .then((json) => {
    ...
    this.setState(
      {
        guide: [],
        loading: true,
        data: "Stiamo sottoponendo la tua domanda a GPT, attendi qualche secondo...",
      },
      () => {
        this.callGPT();
      },
  ...
  removeLike() {
    fetch(
      "http://localhost:4200/api/guides/like/remove/" +
        ...
        "&" +
        this.state.dati_guida +
        "&" +
        this.state.currentTitle,
    ).then(this.setState({ liked: "Mi piace" }));
  }

  reqGuide() {
    this.setState({
      data: "Abbiamo preso in carico la tua richiesta. Controlla nei prossimi giorni perche' qualcuno potrebbe aver scritto una guida a riguardo",
      loading: false,
    });
  ...
  showGuide(event) {
    this.setState({
      clicked: true,
      dati_guida: event.currentTarget.dataset.autore,
      currentTitle: event.currentTarget.dataset.titolo,
        ...
        if (this.state.liked === "Piaciuto")
          like = (
            <input
              ...
              onClick={(event) => this.removeLike(event)}
            ></input>
          );
        else
          like = (
            <input
              type="button"
              value={this.state.liked}
              onClick={(event) => this.addLike(event)}
            ></input>
          );
      }
    }
    ...
    if (this.state.clicked === true) {
      dati = (
        <aside id="guida_dati">
          ...
          <pre>
            <span>Autore: {this.state.dati_guida}</span>
          </pre>
          {like}
          <input
            type="button"
    ...
    if (localStorage.getItem("Email") == null)
      menu = (
        <aside id="menu">
          ...
          <div className="sec">
            <Link to="/writers">Iscriviti come scrittore</Link>
          </div>
          <div className="sec">
            <Link to="/login">Accedi</Link>
          </div>
          <div className="sec">
            <Link to="/wlogin">Accedi come scrittore</Link>
          </div>
        </aside>
      );
    ...
    return (
      <div className="homepage">
            ...
            <div id="spazio_guide">
              {this.state.guide.map((guida, index) => (
                <Guida
                  ...
                  titolo={guida.titolo}
                  testo={guida.testo}
                  onClick={(event) => this.showGuide(event)}
                />
              ))}
            </div>
        ...
        <footer>
          ...
          <pre>
            {" "}
            <em>Carontech</em>, una guida verso il futuro{" "}
