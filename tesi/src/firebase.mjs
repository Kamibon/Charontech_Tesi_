...
import {
  ...
  signOut,
  updateEmail,
} from "firebase/auth"; //'https://www.gstatic.com/firebasejs/9.6.3/firebase-auth.js';
import {
  addDoc,
  collection,
  deleteDoc,
...
async function fetchGuide(app, lista) {
  ...
  for (let el of lista) {
    const q = await query(
      collection(db, "Guide"),
      where("autore", "==", el.payload.autore),
      where("titolo", "==", el.payload.titolo),
    );
...
const updateAuthor = async (app, body) => {
  ...
  querySnapshot.forEach(async (document) => {
    ...
    switch (body.campo) {
      ...
      case "Descrizione":
        await updateDoc(doc(db, "autori", id), { Descrizione: body.valore });
    }
  });
};
...
const updateGuide = async (app, body) => {
  ...
  const q = await query(
    collection(db, "Guide"),
    where("titolo", "==", body.titolo),
    where("autore", "==", body.autore),
  );
  //,
  //
  const querySnapshot = await getDocs(q);
